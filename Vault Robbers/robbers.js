...
//create robbers, assign each iniital vault time from vaul they are going to start on
for(let i = 0; i < R; i++){
	let newRobber = {
		id: i,
		vaultSolveTimer: vaults[i],
		noMoreVaultsToCrack: false
	}
...
while(solvedVaults < V) {
	for(let i = 0; i < R; i++){
		if(counter === robbers[i].vaultSolveTimer && robbers[i].noMoreVaultsToCrack === false){
			...
			robbers.forEach(r => {
				printErr(r.id, r.noMoreVaultsToCrack, r.vaultSolveTimer)
			})
